\section{Anwendungsfälle und Anforderungen}
Im Folgenden werden zwei typische Anwendungsfälle (Use Cases) aufgeführt.\\ \textbf{Einmalige
Anmeldung eines Benutzers:}
\begin{enumerate}
  \item Ein Benutzer meldet sich an, indem er seine Kundennummer und seine Postleitzahl eingibt und
  auf einen Button "`Anmelden"' drückt.
  \item Beim Backend werden die Angaben des Benutzers überprüft \begin{enumerate}
    \item Erfolgreich: Die App wird "`frei geschaltet"'.
    \item Fehler: Der User muss seine Daten prüfen und erneut versuchen.
  \end{enumerate}
\end{enumerate} 
\textbf{Bestellung abschießen:}
\begin{enumerate}
  \item Der User füllt seine persönlichen Daten (Name, Telefonnummer, Emailadresse) aus und
speichert die.
  \item Der Benutzer scannt einen Barcode.
  \item Die Barcodenummer wird beim Server überprüft \begin{enumerate}
    \item Ein Artikel mit dieser Nummer wird gefunden und es erscheint eine Beschreibung des
    Produkts auf dem Bildschirm des Smartphones.
    \item Im Falle, dass es keinen Artikel in der Serverdatenbank gefunden wird, erscheint eine
    Fehlermeldung auf dem Bildschirm.
  \end{enumerate}
  \item Der Benutzer gibt die gewünschte Menge an und fügt, durch Klick auf
einen Einkaufswagen-Toolbarbutton, den Artikel in den Warenkorb ein.
  \item Im Warenkorb hat der Benutzer die Möglichkeit eine der Bestellpositionen auszuwählen und den
  entsprechenden Artikel zu seiner Favoritenliste hinzuzufügen.
  \item Bevor der User die Bestellung abschickt, hat er die
Möglichkeit, einen Wunschliefertermin einzugeben oder eine Notiz, wie z.B. andere Lieferadresse
mitzusenden.
  \item Der User drückt den "`Absenden"'-Button und die Bestellung wird gesendet.
  \item Die Bestelldaten werden beim Server überprüft. \begin{enumerate}
    \item Der Server meldet, dass die Bestellung erfolgreich abgeschlossen wurde.
    \item Der Server meldet, dass ein Fehler aufgetreten ist
  \end{enumerate}
  \item Der User kann sich die abgeschlossenen Bestellungen, sowie die Liste aller gescannten
Barocodes ansehen.
\end{enumerate}
Aus diesen Anwendungsfällen sind mehrere Anforderugen an die App entstanden. Neben triviale
Anforderungen, wie bspw. die App soll nummerische, sowie alpha-nummerische Eingabefelder anbieten,
gibt es einige sehr anspruchsvolle und für die Evaluierung des
Cross-Plattform-Entwicklung-Frameworks interessante Anforderungen.
Zum Beispiel: Die Applikation soll mittels Web Services mit dem Server kommunizieren können.
Die App soll mithilfe des Verschlüsselungsalgorithmus SHA-256 einen Adresszusatz für die Web
Services URI generieren können, um eine Kommunikation mit dem Backend zu ermöglichen.
Die Anwendung soll auf die Kamera, inkl. das Blitzlicht des Gerätes zugreifen können. Die Daten
sollen persistiert werden können.
...
\begin{itemize}
  ...
  anwendungsspezifische Datentypen beschreiben. Im Model sind Klassen zur Speicherung und
  Verarbeitung von Daten zu finden, die dafür sorgen, dass die Daten konsistent gehalten
  werden.
...
Page())}.\\Nach dem MVVM-Muster sollten sich die Viewmodels um die Navigation kümmern. Allerdings
ist \textit{Navigation} ein Property der Ansicht. D.h. das Viewmodel muss signalisieren,
