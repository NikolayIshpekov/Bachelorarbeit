...
\begin{itemize}
  \item ScanApp - das gemeinsame, plattformübergreifende Shared Projekt.
...
Datenbank in der Ressourcen des ScanApp.Droid Projekts) ließen sich mittels Compiler Direktiven und
...
oder "`Forms ContentView Xaml"' an. \\Wie bereits im \ref{View} erläutert, wurden die
...
Xaml-Datei zu sehen, die die in Abb. \ref{fig:abb34} dargestellte "`Persönliche Daten Ansicht"'
...
Während Unit-Tests die App-Logik testen, werden bei UI-Tests Funktionstests der UI
Steuerelemente durchgeführt. Somit wird sichergestellt, dass die App, einschließlich
Bedienoberfläche ordnungsgemäß funktioniert. \\Das MVVM Architekturmuster ist besonders gut für
automatisierte Unit Tests geeignet. Da die App-Logik in den ViewModels gekapselt ist, lassen sich
Unit Tests für jeden ViewModel entwickeln. Auf diese Weise kann die Funktionalität eines ViewModel
getestet werden, ohne die View kennen zu müssen.\\ Wie bei jeder mobilen Applikation steht
Interaktion mit dem Benutzer im Vordergrund bei der Scan-App. Viel interessanter für die App
Entwicklung sind die UI-Tests, da mit einem UI Test ein Use Case
nachgestellt werden kann und dadurch kann die App Funktionalität nach Korrektheit getestet werden.
...
in Wirklichkeit Text in ein Entry-Feld eintippen, irgendeine Geste durchführen, oder auf einen
Button klicken usw.
...
Fällen werden sie bei der Installation von Xamarin.Android mitinstalliert [\cite{XamUITest}].
...
Jailbreak) zur Verfügung. Xamarin Test Cloud generiert bei jedem Schritt eines Testfalls Screenshots
...
Geräten ziehen und gegebenfalls visuelle Inkonsistenzen feststellen. Screenshots können allerdings
nicht alle Benutzereindrücke abbilden. Für diesen Zweck bietet der Service auch
...
\\Alle Xamarin-Plattform-Abonnements beinhalten 60 Xamarin Test Cloud Geräte-Minuten pro
...
% eines UI-Tests auf dutzende Geräten verglichen werden. Der Entwickler hat die Möglichkeit
