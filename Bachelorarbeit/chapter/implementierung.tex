...
\begin{itemize}
  \item ScanApp - das gemeinsame Projekt.
  \item ScanApp.Droid - plattformspezifisches Projekt für Android.
  \item ScanApp.iOS - plattformspezifisches Projekt für iOS.
\end{itemize}
Der Großteil des Codes befindet sich im gemeinsamen Projekt. In die plattformspezifischen Projekte
gehören App-Ressourcen wie bspw. Icons, sowie benutzerdefinierte Renderer. Die sogenannten "`Custom
Renderer"' werden für die Implementierung und Gestaltung von benutzerdefinierten
Steuerelementen und Ansichten benötigt, die mit den von Xamarin.Forms zur Verfügung gestellten
Techniken nicht zu realisieren sind. \\Sollte eine Funktionalität plattformspezifische APIs
erfordern, wird ein Interface im gemeinsamen Projekt definiert und wird von
den jeweiligen Klassen der plattformspezifischen Projekte implementiert.
Der Zugriff aus dem gemeinsamen Projekt erfolgt folgendermaßen:\\
...
abgeleitet wird. In Base\_Page werden Eigenschaften definiert, die für die meisten Ansichten
gemeinsam sind, sowie bspw. die Steuerung der Navigation. \\Eine der wichtigsten Eigenschaften der
Klasse Xamarin.Forms.Page ist das Property \textit{BindingContext}. Mithilfe dieser Eigenschaft wird
ein Viewmodel (in diesem Fall das HeaderDataEditViewModel) an der Page-Klasse gebunden (Siehe
\ref{fig:abb23}, Zeile 23-25).
\\In Zeile 12 findet eine typische Datenbindung statt.
...
\\Ein ViewModel muss das Interface \textbf{\textit{INotifyPropertChanged}} implementieren.
Dadurch können sogenannte "`Bindable Properties"` definiert werden (siehe Abb.\ref{fig:abb33}).
\\Wie bei den meisten Applikationen, sind für die Scan-Anwendung mehrere Ansichten
erforderlich. Es stellt sich die Frage, wie wird zwischen den verschiedenen Seiten gewechselt?\\Um
zwischen den Ansichten navigieren zu können, wird eine Instanz der Klasse
