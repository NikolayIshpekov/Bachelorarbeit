...
Methoden zum Code-Sharing:
 Shared Projects und Portable Class Libraries (PCL). Da wir Code-Sharing nur innerhalb der Nüßing
 App beabsichtigen haben wir uns für Shared Projects entschieden (der PCL Ansatz wird für den Fall
 empfohlen, wenn der Entwickler seinen Code für andere Developer in Form einer DLL Bibliothek bereitstellen möchte).
Beim Erstellen des Xamarin.Forms Projekts namens ScanApp wurden automatisch zwei weitere Projekte
erstellt, so dass man drei verschiedene Projekte hat:
\begin{itemize}
...
validen Pfad zu konstruieren.
 Da beim gemeinsamen ScanApp Projekt keine Bibliotheken eingebunden
werden können muss man den
\subsection{GUI}
...
zugrundeliegenden (Code-Behind) C\#-Datei implementiert) erreicht. Darüber hinaus ermögilcht dieser
Ansatz, dass Designer und Entwickler unabhängig voneinander arbeiten können.\\Bei der Erstellung
einer Xamarin.Forms ContentPage Xaml-Datei wird automatisch auch die zugrundeliegende C\#-Datei,
sogenannte Code-Behind-Datei, angelegt (siehe Abb. \ref{fig:abb21}).
...
\caption{Seite zum Editieren der persönlichen Daten eines Nutzers der ScanApp}
\label{fig:abb24}
\end{figure}
\begin{figure}[!h]
\centering
