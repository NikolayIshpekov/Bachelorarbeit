...
Bei der App-Branche wird eine steile Wachstumskurve beobachtet [\cite{Statista}]. Die Vielfalt von
mobilen Anwendungen ist groß. Von hochspezialisierten Business-Apps bis hin zu Game-Apps. 
Für die meisten Unternehmen ist es mittlerweile zu einer Selbstverständlichkeit geworden, eine oder
sogar mehrere Apps anzubieten [\cite{CompWoche}]. Es entsteht die Frage, wie
wird eine mobile Anwendung erstellt?\\In diesem Kapitel werden die für das Verständnis notwendigen Grundlagen
der App Entwicklung erläutert.
% Die Themen werden stark komprimiert und vereinfacht vorgestellt.
\section{Aktuelle mobile Plattformen}
Mobile Betriebssysteme sind fest an ein bestimmtes Smartphone gebunden. 
...
Im Folgenden werden die aktuellsten mobilen Betriebssystemen vorgestellt.
\begin{itemize}
  \item \textbf{Android}\\Das mobile Betriebssystem Android wurde von Google entwickelt und im
  November 2007 der Öffentlichkeit vorgestellt. Es besteht aus einem Betriebssystemkern, Middleware
  und eigenen Applikationen (Telefonbuch, Kalender etc.). Da wesentliche Teile der Android Plattform
  unter einer Open Source Lizenz (Apache v2.0) stehen, kann Android von Lizenznehmern ohne Zahlung
  von Lizenzgebühren lizenziert und wegen der quelloffenen Verfügbarkeit nach eigenen Bedürfnissen
  angepasst werden. Neben der Bereitstellung von Programmierschnittstellen bietet Android auch
  umfassende Quellcodesammlung für Entwickler an. Der ebenfalls quelloffene Betriebssystemkern von
  Android basiert auf Linux 2.6.
  Über "`Play Store"' (ein zentrales Softwareverzeichnis) lassen sich Applikationen kaufen und auf
  dem Gerät installieren. Google übernimmt für den Softwareentwickler die Distribution der Software
  und sorgt für eine kostenlose Verteilung von Updates. Das Inkasso erfolgt durch ein Zahlsystem von
  Google, wobei Google 30\% als Provision behält und dem Entwickler monatlich 70\% des
  ...
  Betriebssystems Mac OS X und wurde zusammen mit dem iPhone im Juni 2007 vorgestellt. Die
  Installation von iOS ist nur auf Apple-Geräte möglich, weil Apple im Gegensatz zu Google iOS nicht an andere Gerätehersteller
  lizenziert.
  Zum Entwickeln von iOS-Applikationen wird das iOS-SDK (Software Development Kit) benötigt. Da das
  iOS-SDK Apple Rechner mit Intel-Prozessoren voraussetzt, können alte Macs mit PowerPC-CPUs nicht zum
  Entwickeln verwendet werden.
%   Im Laufe der Zeit wurde das Betriebssystem und der damit verbundene
%   Funktionsumfang sukzessive dem technologischen Fortschritt angepasst und 
  Das Betriebssystem beinhaltet einen
  E-Mail-Client, ein Programm zur SMS-Verwaltung, eine Möglichkeit, kabellos zu drucken, einen
  Browser, ein Werkzeug zur Sprachsteuerung sowie eine Multimediaanwendung, um Videos und Bilder
  sowohl selbst zu erstellen als auch abzuspielen. Ähnlich wie bei Android lassen sich Applikationen
  nur durch Download über ein zentrales Softwareverzeichnis von Apple ("`App Store"') auf einem
  Gerät installieren. Die Installation von unsignierten
  Applikationen wird von Apple nicht zugelassen. Aus diesem Grund ist es nicht möglich unter
  Umgehung des App Stores, Programme auf das Gerät zu installieren. Ähnlich wie bei Android
  übernimmt der App Store eine Marktplatz-, Distributions-, Wartungs- und Inkassofunktion.
  ...
  bei Android behält Apple eine Verkaufsprovision in Höhe von 30\%. 
%   Ein Vorteil von iOS
%   gegenüber Android ist die nicht so große Vielfalt in Bezug auf Displaygröße. 
  Im Vergleich zu Android Geräten gibt es bei Apple keine so große Vielfalt in Bezug auf
  Bildschirmgröße des Smartphones. Aus Sicht des Entwicklers ist das ein Vorteil, weil dadurch die Entwicklung, Wartung, Test und
  Erweiterung von iOS Apps erleichtert wird.
%   Diese durchgängige Standardisierung  vereinfacht die Entwicklung, Wartung, Test und
%   Weiterentwicklung von iOS Apps aus Sicht des Entwicklers. 
  Zum Testen auf Endgeräten und für die Veröffentlichung von Apps im Apples
  ...
  werden.
  \item \textbf{Windows Phone}\\Microsoft bietet mit Windows Phone ein Betriebssystem und
  einige Zusatzprogrammen (Outlook, Word mobile etc.) für Smartphones. Windows Phone ist die
  Weiterentwicklung von Windows Mobile und im Gegensatz zu seinem Vorgänger, nicht nur einseitig an
  Business-Kunden ausgerichtet ist, sondern auch an Consumer-Kunden.  
%   Während Windows
%   Mobile einseitig an Business-Kunden ausgerichtet war, hat Windows Phone Geschäfts- wie Consumer-Kunden im Auge. 
%   Im Gegensatz zu Microsoft Windows für stationäre Desktop-PCs, das nahezu ausschließlich auf
%   Intel-x86-Prozessoren läuft, ist Windows Phone auf vielen unterschiedlichen Prozessorarchitekturen
%   lauffähig.
  Während Microsoft Windows für stationäre Desktop-PCs nahezu ausschließlich auf
  Intel-Prozessoren läuft, ist Windows Phone flexibler und auf vielen unterschiedlichen
  Prozessorarchitekturen lauffähig.
  Trotz der oft begrenzten Hardwareressourcen von mobilen und embedded Geräten bietet Microsoft ein
  Smartphonebetriebssystem mit einer akzeptablen Systemleistung.
  Seit dem Erscheinen des Betriebssystem Windows Phone setzt Microsoft ein Kachelkonzept. 
  Neben Funktionalitäten wie SMS-Verwaltung, Telefonfunktion, Kalender und soziale Netzwerke,
  arbeitet Windows Phone nach dem Prinzip des Cloud-Computing. Dienste wie Facebook werden bspw.
  integriert und mit den Funktionen des Betriebssystems verknüpft. Damit Apps auf einem Windows
  Phone Gerät installiert und getestet werden können, wird vorausgesetzt, dass das Gerät
  explizit als Entwicklergerät freigeschaltet worden ist.
  Dazu ist eine Registration bei Microsoft als Entwickler erforderlich. Diese Registration kostet 99
  EUR im Jahr und wird auch für das Veröffentlichen von Apps im Microsoft Market (vergleichbar mit
  App Store und Play Store) benötigt
%    Dazu
%   muss man sich zuerst bei Microsoft als Entwickler registrieren, was 99 EUR im Jahr kostet.
%   Das ist auch notwendig, wenn man Apps im Microsoft Market (vergleichbar mit App Store und Play Store) veröffentlichen möchte 
  [\cite[17]{AppsPhoneGap}].
  \item \textbf{Blackberry OS}\\Das Betriebssystem wird von Research In Motion (RIM) für die
  Nutzung auf Smartphones des Typs Blackberry hergestellt. 
  Ähnlich wie bei iOS von Apple ist es ausschließlich für Blackberry-Smartphones verfügbar.
%   für Blackberry-Smartphones vorgesehen. 
  Da Blackberry-Smartphones vorwiegend auf
  Geschäftskunden ausgerichtet sind, bieten sie komfortable Synchronisationsmechanismen für Daten
...
Der Begriff Applikation oder auch App hat sich in Bezug auf die mobile Nutzung und
Anwendung als eigenständiger Begriff etabliert. Eine App wird als \textit{nativ} bezeichnet, wenn
sie für eine bestimmte mobile Plattform entwickelt worden ist und nur auf dieser Plattform
installiert und ausgeführt werden kann. Die Vorteile einer nativen App sind eine gute Performanz
und vor allem, die Möglichkeit zum Zugriff auf besondere Hardwarebestandteile der mobilen Endgeräte.
Ebenso können viele vorgefertigte Features aus nativen Schnittstellen und APIs (application user
interface) benutzt werden.
Da die verschiedenen Verzeichnisdienste, wie App Store, Play Store oder Microsoft Market, native Apps
einfordern, kommt man um eine echte Anwendung nicht herum, wenn ein kommerzieller Vertrieb
einer App angestrebt wird.\\Ursprünglich stand Informationssuche und Nachrichtenübermittlung 
im Fokus des mobilen Internets.
...
Komplexe Anwendung & 2013  & 802  & 48804 & 382215 \\ 
\hline
\end{tabular}
...
\subsection{Business-Apps}
Eine Business-App oder Enterprise-App ist eine App der Komplexitätsstufe "`komplexe Anwendung"'.
Business-Apps zeichnen sich dadurch aus, dass sie meistens auf Geschäftskunden ausgerichtet sind.
Die Bedienoberfläche wird simpel und ohne spezielle visuelle Effekte gehalten.
Viel wichtiger ist die Funktionalität der App.
Mit Enterprise-Apps wird eine Optimierung von Geschäftsprozessen gezielt. 
% Ziel solcher Apps ist es, Geschäftsprozesse zu optimieren und 
Dadurch ist die fehlerfreie und sichere Verarbeitung von sensiblen Daten von höchster Priorität.
\newpage
...
werden muss.\\Das Entwickeln der Bedienoberfläche (auf Englisch User Interface oder kurz UI) einer
...
eine Bedienoberfläche eng mit dem zugrunde liegenden System verbunden ist und die nicht immer
voraussagbaren stilistischen Ansprüche ihrer Benutzer entgegenkommen muss, kann dies der wichtigste
Bereich vieler Anwendungen sein.
Hierfür können Architekturmuster Abhilfe schaffen [\cite[195]{App4U}].
...
\begin{itemize} 
  \item Die Daten, die auf dem Bildschirm angezeigt werden, sind das \textbf{Model}. 
%   Das Model
%   enthält bis auf Such- und Sortierfunktionen keine Geschäftslogik. 
  Diese Schicht kapselt die Daten
...
\end{itemize} 
In der Controller-Schicht wird die Geschäftslogik der Anwendung und die Steuerung des Programms
untergebracht.
...
und jede View lässt sich in vollkommen unterschiedliche Applikationen
einsetzen [\cite{AppsForiPhone}].
\subsection{Model-View-ViewModel (MVVM)}\label{MVVM}
\begin{figure}[!h]
\centering
...
Das \textbf{Model-View-ViewModel} (\textbf{MVVM})-Muster besteht aus drei Komponenten und ist MVC
sehr ähnlich, der Unterschied ist, dass es bei MVVM anstelle von Controller ein sogenanntes
ViewModel gibt (siehe Abb.\ref{fig:abb9}):
\begin{itemize} 
  \item \textbf{Model}. Ähnlich wie bei MVC, kapselt diese Schicht die Daten der Anwendung in
  Klassen und ist für die permanente Speicherung und Verarbeitung der Daten zuständig.%Das Model
  ...
  und der View. 
\end{itemize} 
Es ist ein lose gekoppelter Entwurf und dadurch wird das Unit-Testing erleichtert.
...
Die View wird an die Eigenschaften eines ViewModels gebunden, das wiederum Daten verfügbar macht,
die in Modellobjekten und anderen für diese Ansicht spezifischen Zuständen enthalten sind. Das Erstellen der Bindungen
zwischen Ansicht und ViewModel ist einfach, da
...
Befehl ausgeführt, um die angeforderte Aktion durchzuführen. Das ViewModel führt alle an den Modelldaten vorgenommenen 
Änderungen durch. Die View-Klassen wissen
...
ViewModel-Klassen durchzuführen [\cite{MSDNAMVVM}].
\newpage
...
Mobile Anwendungen werden als nativ bezeichnet wenn, die auf einem bestimmten Endgerätetyp und dessen zugehörigen
Betriebssystem lauffähig sind. Die Entwicklung nativer Apps ist dann sinnvoll,
...
Als erstes wird das
  Java Development Kit (JDK) benötigt. Das JDK dient zum Kompilieren der
  Java-Textdateien in .class-Dateien. Ebenso wird noch das Android Software Development
  Kit (SDK) vorausgesetzt.
  Es gibt mehrere integrierte Entwicklungsumgebungen (oft auch als IDE
  (integrated development environment) bezeichnet), mit denen man Android Apps entwickeln kann. Die
  können auch auf allen Plattformen erstellt werden, auf denen Java-Entwicklung möglich ist. Die Entwicklungsumgebung Eclipse hat sich als Standard für
...
auch für Mac OS. Die Installation von Eclipse ist unproblematisch, wenn auf einem Rechner
bereits ein JDK installiert ist. Eclipse wird als komprimierte Datei ausgeliefert, die einfach
extrahiert werden muss. Beim ersten Start sucht sich die IDE die passende Java-Umgebung und richtet
...
Möglichkeit, Teile der Applikation in nativen Programmiersprachen zu implementieren.
Es lassen sich bspw. C oder C++ Bibliotheken mittels NDK verwenden.
\\Das Android-SDK kompiliert den Java-Code und packt das Programm und alle anderen zugehörigen
Ressourcen, wie Layoutdateien und Grafiken, in eine .apk-Datei.
...
Entwicklungswerkzeugen automatisiert, so dass keine zusätzlichen Arbeitsschritte aus Sicht des
Entwicklers anfallen. Jede Anwendung in Android wird in einer eigenen virtuellen Maschine, der
sogenannten Sandbox, ausgeführt. Auf diese Weise wird sichergestellt, dass jede App isoliert
von den anderen Apps ist und nur auf ihre eigene Daten zugreifen kann. So kann die Stabilität des
Systems nicht durch den Absturz einer App gefährdet werden.
%  wird unter anderem bei einem Absturz die
% Stabilität des Systems gewährleistet.
Die Entwicklung von Applikationen für Android erfolgt nach dem Model-View-Controller
Architekturmuster. Im Folgenden werden die wichtigsten Komponenten einer Android Applikation
vorgestellt [\cite[317]{App4U}].
\begin{figure}[!h]
\centering
...
AndroidManifest.xml. In dieser Datei werden globale Beschreibungen festgelegt, die für die App
gelten.
\begin{itemize}
...
dar, welches Eingaben vom Benutzer erwartet. Jede Activity besitzt eine eigene
Bedienoberfläche und kann andere Activities starten.
...
\begin{itemize}
  ...
  Benutzungsoberfläche zuständig. In der onCreate() Methode wird die setContentView Funktion
  aufgerufen, durch die der Activity entweder eine View, oder direkt ein Layout über die Ressourcen ID
  zugewiesen wird.
...
werden, in den Intent gespeichert werden, bevor die neue Activity gestartet wird
[\cite[321]{App4U}].
\subsubsection{Komponenten der Benutzungsoberfläche}
Die Benutzungsoberfläche oder User Interface (UI) in Android Apps besteht aus Views und View Groups.
...
\subsection{iOS}
Für die Entwicklung von iOS Apps wird Apples kostenlose Entwicklungsumgebung Xcode benötigt,
welche ausschließlich für Apple Rechner zur Verfügung steht. Xcode bringt das aktuelle iOS SDK mit.
In dem iOS SDK sind hochwertige Emulatoren für die aktuelle Version
 ...
 Benutzungsoberfläche zu einer relativ einfachen Aufgabe.
 ...
 \\Ähnlich wie bei Android wird das MVC-Architekturmuster für die Entwicklung von iOS Apps
 verwendet.
 ...
 Endgeräte abgestimmte Version des Frameworks zur Verfügung. Analog zu Android werden Apps in einer beschränkenden
 Sandbox ausgeführt und es
...
die Rechte aktiv bestätigen. \\Im Folgenden werden die wichtigsten Komponenten der iOS App
Entwicklung vorgestellt.
\\Um für die View-Komponenten ein Verhalten zu definieren, stehen drei Mechanismen zur Verfügung:
\begin{itemize}
  \item \textbf{Delegation}. Ermöglicht eine entkoppelte Kommunikation. 
  \item \textbf{Target Action}. Über den Target-Action-Mechanismus benachrichtigt die View den
  Controller, wenn bspw. ein Button geklickt wurde.
  \item \textbf{Notification}. Dieses Mechanismus ermöglicht es, dass Objekte informiert werden,
  sobald sich ein bestimmtes Property eines anderen Objektes geändert hat.
   Die Kommunikation vom Model zum
  Controller erfolgt über Notifications und Key-Value-Observing.
\end{itemize}
...
Benutzungsoberfläche Property Lists bearbeiten kann, ohne sich mit XML Code auseinander setzen zu
...
einer microsofteigene Entwicklungsumgebung, nämlich Visual Studio, erstellen.
Für die Installation von Visual Studio wird das Betriebssystem Windows von Microsoft vorausgesetzt. 
Wie Xcode und Eclipse stellt Visual Studio eine integrierte Entwicklungsumgebung zur Verfügung. Im Gegensatz zu den
iOS- und Android-Entwicklungsumgebungen ist Visual Studio bis auf die funktional
beschränkten Express-Versionen kostenpflichtig. Es wird das kostenlose Windows Phone SDK vorausgesetzt. Nach der
Installation des SDKs, werden die Tools für die Windows Phone-Entwicklung automatisch
in die bestehende Entwicklungsumgebung Visual Studio integriert. 
Visual Studio bietet Entwurfansicht zur Darstellung der Bedienoberfläche und Texteditor für die
Eingabe des Quellcodes an.

Für die Erstellung einer nativen Windows Phone App kann C\#, Visual
Basic und XAML oder C++ und DirectX verwendet werden. Es lassen sich auch in Internet Explorer
ablaufende Windows Phone-Apps mit HTML5, JavaScript und CSS entwickeln.
...
Verarbeitungslogik definieren. Es entsteht einen XAML-Code \footnotemark[1]. Diese
...
C\#-Datei, die genau so heißt (Datei.xaml.cs) und als "`Code-Behind-Datei"' bezeichnet wird.
Beim Deklarieren eines Oberflächeelements im XAML kann man auf dieses Element aus der
Code-Behind-Datei zugreifen. Jede solche Datei wird an ein ViewModel (C\#-Klasse) gebunden, in dem
die Verarbeitungslogik enhalten ist.
%  In dieser Methode ist dann die Verarbeitungslogik enthalten.
\footnotetext[1]{XAML (Extensible Application Markup Language) ist eine Dialektform von XML.}  
[\cite{MSDNApps}]
[\cite[125]{App4U}]
[\cite[382]{MoApp1}]
\subsection{Fazit}
Abschließend lässt sich feststellen, dass, die nativen
Entwicklungsansätze sich grundsätzlich in den Programmiersprachen und den plattformspezifischen
APIs unterscheiden. Was alle Entwicklungsansätze gemeinsam haben, sind die angewendeten
Architekturmuster - das MVC oder das ähnliche MVVM.\\Da Apps ähnliches Grundgerüst haben, wäre es möglich, mittels eines
