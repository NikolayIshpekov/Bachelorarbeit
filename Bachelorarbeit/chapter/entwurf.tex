...
Aus der Perspektive eines Benutzers fühlt sich eine Xamarin.iOS App wie jede native iOS App an und
jede Xamarin.Android App.wie eine nativ entwickelte Android App. 
Dank Xamarin.Forms werden die Ansichten komplett plattformunabhängig definiert. 
...
"`Cross-Plattform-Entwicklung"' gezielt, dass alles, was sich auslagern lässt, in das gemeinsame
Projekt gehört. Die plattformspezifischen Projekte sollten möglichst "`leer"' bleiben.
...
% \label{fig:abb27}
% \end{figure}
\subsection{Shared ScanApp Projekt}
...
gegeben. Wie bereits im Kapitel \ref{MVVM} erörtert, weist das MVVM-Pattern eine lose Kopplung
zwischen View und Viewmodel auf. Um diese Trennung visuell zu gestalten, werden drei Ordner erstellt:
\subsubsection{Model}
...
Benutzungsoberfläche ist kein Pflicht, allerdings ist der C\#-Code schwerer lesbar als der
...
Steuerelemente des dazugehörenden Views gebunden werden. \\Die Klassen der ViewModel-Komponente
...
\ref{Navigation} beschrieben wird, zuständig. Da nach MVVM die View den ViewModel kennt, nicht aber
andersrum, kann eine ViewModel-Klasse ausschließlich durch das \textbf{\textit{Xamarin.Forms Messaging Center}} mit der entsprechende View-Klasse
"`kommunizieren"'.
\subsubsection{Util}
In dieser Komponente werden einige nützliche Klassen definiert: 
\begin{itemize}
  \item UtilKlasse mit mehreren statischen Methoden, die von den Klassen des Viewmodels benutzt
  werden.
  \item Klasse mit App-Konstanten. Hier werden üblicherweise vorwiegend Strings ausgelagert.
...
untergebracht wird (Beim Erstellen einer Xaml-Datei wird automatisch auch die dazugehörende
C\#-Code-Behind-Datei erstellt).
Das entspricht, allerdings, nicht dem MVVM-Architekturmuster.
Dadurch würde eine sehr enge Kopplung zwischen View und Funktionalität entstehen und die Ansichten wären
...
\subsection{Datenpersistenz und Backend-Kommunikation}\label{PersistenzBackend}
...
Projekt muss der Pfad zu der SQLite-Datenbank angegeben werden, einmal für Android und
einmal für iOS. Die Zugriffe auf die Datenbank erfolgen plattformübergreifend (d.h. aus dem
"`shared"' Projekt).
...
\subsection{Entwurf der Seiten-Navigation}\label{Navigation} 
...
Funktionalitäten betrachten, die sich im gemeinsamen "`shared"' Projekt implementieren lassen. Die
meisten Funktionalitäten lassen sich auf diese Weise kapseln und "`cross-plattform"'
implementieren, jedoch gibt es Situationen, in denen ein Feature nicht plattformübergreifend
