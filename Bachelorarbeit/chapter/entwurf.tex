...
Der Großteil des Codes befindet sich im plattformübergreifenden Projekt, ScanApp. In diesem
...
das native Pedant(ListView bei Android und UITableView bei iOS) übersetzt.\\In die Projekte
...
ausreichend sind, so dass ein benutzerdefinierter Renderer selten benötigt wird. Die
Hintergrundfarbe einer markierten Zeile eines ListViews ist bspw. blau bei der Android Version und grau bei der iOS Version der App. Obwohl die Ansichten im "`shared"' Projekt erstellt worden sind, sehen sie nicht gleich aus. Da der gemeinsame Code zum nativen Code kompiliert wird, tragen die auf diese Weise erzeugten
Steuerelemente spezifische Merkmale der jeweiligen Plattform.
Sollte die Hintergrundfarbe bei Android auch grau sein, wäre ein Custom Renderer im
plattformspezifischen Droid-Projekt nötig. An die plattformspezifischen Projekte lassen zwei
Arten von "`open source"' Bibliotheken einbinden: Xamarin Komponenten und NuGet Packages. Eine
solche Xamarin Komponente ist bspw. die ZXing-Komponente zum Scannen von Barcodes, die im
vorliegenden Fall eine zentralle Rolle in der App spielt. Das ebenso in der ScanApp eingebundene
NuGet Package "`DeviceInfo"' bietet eine API, mit der Entwickler Informationen über das
Smartphone gewinnen können. 
\begin{figure}[!h]
\centering
\includegraphics[scale = 0.435]{graphics/ScanAppProjekt.png}
